...
def run_lime_tabular(X, classifier, feature_names, class_names):
    ...
    # Generate synthetic training data for LIME (it needs a reference dataset)
    # Create variations around the sample point
    np.random.seed(42)
    n_samples = 500
    n_features = X.shape[1]

    # Generate training data with similar distribution
    training_data = np.random.randn(n_samples, n_features)
    # Add the actual sample to ensure it's represented
    training_data = np.vstack([X, training_data])

    # Create explainer with synthetic training data
